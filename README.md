...
Wheeler Memory encodes text into a 64x64 cellular automaton (CA) grid, evolves it through 3-state dynamics until it converges to a stable attractor (~40-50 ticks), and stores the resulting pattern. Similar concepts produce similar attractors. When you query, your input is evolved the same way and matched against stored attractors via Pearson correlation.
